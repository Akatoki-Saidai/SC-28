...
# RPi.GPIOを排除し、すべてgpiozeroに統一した修正版
#---------------------------------------------------------------------
...
# ---------------------------------------------------------
def setup_gpio():
    """LEDやVM用のGPIO初期化 (gpiozeroに統一)"""
    global _gpio_initialized, led_device, vm_device, _factory
    if not _gpio_initialized:
        try:
            if _factory is None:
                _factory = PiGPIOFactory()
            
            # factoryを共有して安全にピンを確保
            led_device = LED(PIN_LED, pin_factory=_factory)
            vm_device = OutputDevice(PIN_VM, pin_factory=_factory)
...
def cleanup():
    """終了時の安全停止処理"""
    global motor_left, motor_right, led_device, vm_device
...
def move(direction, power, duration, is_inverted=False, enable_stack_check=True):
    ...
    """
    global motor_right, motor_left, bno, vm_device, _gpio_initialized
    ...
    # 移動開始時にVMを有効化
    if _gpio_initialized and vm_device is not None:
        vm_device.on()
    ...
    if remaining_time > 0:
        ...
        if duration >= 2 and bno is not None and enable_stack_check:
            ...
            while time.time() - start_t < remaining_time:
                ...
                gyro = ijochi.abnormal_check("gyro", lambda: bno.gyroscope(), ERROR_FLAG=False)
                lin_accel = ijochi.abnormal_check("accel_line", lambda: bno.linear_acceleration(), ERROR_FLAG=False)
